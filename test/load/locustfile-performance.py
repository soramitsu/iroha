...
class IrohaClient(IrohaGrpc):
    ...
    def await_status(self, transaction, start_time):
        ...
        try:
            ...
            events.request_failure.fire(request_type="grpc", name='send_tx_await', response_time=total_time, exception=e, tx_hash=hex_hash)
        else:
            total_time = int((time.time() - start_time) * 1000)
            events.request_success.fire(request_type="grpc", name='send_tx_await', response_time=total_time, response_length=0, tx_hash=hex_hash)
